...
import 'package:royalmart/model/ShopDModel.dart';
...
Future<List<WalletUser>?> get_walletrecord2(String val, int lim) async {
  String link = FoodAppConstant.base_url +
      "manage/api/wallet_user/all?X-Api-Key=" +
      FoodAppConstant.API_KEY +
      "&start=" +
...
Future<List<MembershipPlan>?> getMembershipsList() async {
  String link = FoodAppConstant.base_url +
      "manage/api/MembershipPlans/all/?X-Api-Key=" +
      FoodAppConstant.API_KEY +
      "&start=0&limit=&shop_id=" +
      FoodAppConstant.Shop_id +
      "&status=Active&types=user";
  print('Address ${link}');
  final response = await http.get(Uri.parse(link));
  if (response.statusCode == 200) {
    var responseData = json.decode(response.body);
    List<dynamic> galleryArray = responseData["data"]["membershipPlans"];
//    List<UserAddress> list =

    return MembershipPlan.getListFromJson(galleryArray);
  }
//    print("List Size: ${list.length}");
}

...
class PanCardUpload extends StatefulWidget {
  const PanCardUpload({Key? key}) : super(key: key);
...
class _PanCardUploadState extends State<PanCardUpload> {
  @override
  ...
  final data = await ImagePicker().pickImage(source: ImageSource.camera);
    imageshow1 = File(data!.path);    String imagevalue1 = (imageshow1).toString();
    if (imagevalue1 != null) {
...
//    print(mobileController.text);
    try {
      final response = await http
          .post(Uri.parse(GroceryAppConstant.base_url + 'api/kycupload.php'), body: map);
